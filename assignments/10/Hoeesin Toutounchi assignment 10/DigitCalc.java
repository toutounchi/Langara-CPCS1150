...
public class DigitCalc {
    ...
    {
        //This method calculates the number of digits and adds a "," based on the location of the digit.

        int count = 0;
        String sumFinal = "";
        for(int i= sum.length()-1; i>=0;i--)
        {
            //We initially set the counter to zero and increment if its not equals to zero
            if(count!=3)
            {
            ...
            {
                //otherwise We set the counter to 1 and print comma to seperate numbers.
                sumFinal =  sum.charAt(i) + "," + sumFinal;
                count = 1;
            }
        }
    ...
    public static void calculate(String num1, String num2)
    {
        ...
        //We pass each value to commaGenerator method to get the commas in between.

        System.out.println("The output should be:");
        //We generate spaces between our small number and left side to make it look nice.
        for(int i = 0; i< num1.length() - num2.length(); i++)
            System.out.print(" ");
        System.out.println(commaGenerator(num2));
        System.out.println(commaGenerator(num1)+ "+");
        //We generate a line to separete our answer and the numbers.
